...
class ShadowEffects:
    ...
    def apply_shadow(self, image_path: str, output_path: str,
        ...
        logger.info(f"🌟 Aplicando sombra tipo '{shadow_type}' a {Path(image_path).name}")

        try:
            # Cargar imagen con transparencia
            img = Image.open(image_path).convert('RGBA')
            logger.info(f"📐 Imagen cargada: {img.size[0]}x{img.size[1]} pixels")

            # Aplicar sombra según tipo
            if shadow_type in self.shadow_types:
                result_img = self.shadow_types[shadow_type](img, **shadow_params)
                logger.info(f"✅ Sombra '{shadow_type}' aplicada exitosamente")
            else:
                logger.warning(f"⚠️ Tipo de sombra '{shadow_type}' no reconocido, usando original")
                result_img = img.convert('RGB')

            # Las sombras ya incluyen fondo blanco, no necesitamos aplicar otro
            if result_img.mode == 'RGBA':
                logger.warning("⚠️ Sombra devolvió RGBA, convirtiendo a RGB")
                ...
                result_img = white_bg
            logger.info("🎨 Imagen con sombra lista para guardar")

            # Guardar resultado
            result_img.save(output_path, 'PNG', quality=95)
            logger.info(f"💾 Imagen con sombra guardada en {Path(output_path).name}")

            return True

        except Exception as e:
            logger.error(f"❌ Error aplicando sombra: {str(e)}")
            return False
    ...
    def create_drop_shadow(self, img: Image.Image, intensity: float = 0.3,
        ...
        logger.info(f"🎭 Creando drop shadow - intensidad: {intensity}, offset: ({offset_x}, {offset_y}), blur: {blur_radius}")
        ...
        logger.info("🔴 SOMBRA ROJA pegada en canvas blanco")
        ...
        logger.info("📦 Producto pegado encima de la sombra")

        logger.info(f"📏 Canvas final: {canvas.size[0]}x{canvas.size[1]} pixels")
        return canvas
    ...
    def create_reflection_shadow(self, img: Image.Image, reflection_height: float = 0.4,
        ...
        if intensity is not None:
            opacity = intensity
        logger.info(f"🪞 Creando reflection shadow - altura: {reflection_height}, opacidad: {opacity}, intensidad: {intensity}")
        ...
        logger.info(f"🪞 Reflejo creado con altura {reflection_h}px")
        return canvas
    ...
    def create_natural_shadow(self, img: Image.Image, intensity: float = 0.15,
        ...
        logger.info(f"🌿 Creando natural shadow - intensidad: {intensity}, dirección: {direction}")
        ...
        logger.info(f"🌿 Sombra natural aplicada con offset ({offset_x}, {offset_y})")
        return canvas
...
def detect_best_shadow_type(image_path: str) -> str:
    ...
    try:
        ...
        if img is None:
            logger.warning("⚠️ No se pudo cargar imagen para análisis, usando drop shadow")
            return 'drop'

        height, width = img.shape[:2]
        aspect_ratio = width / height

        logger.info(f"🔍 Analizando imagen: {width}x{height}, ratio: {aspect_ratio:.2f}")
        ...
        logger.info(f"🎯 Sombra recomendada: '{shadow_type}' ({reason})")
        return shadow_type

    except Exception as e:
        logger.error(f"❌ Error detectando tipo de sombra: {str(e)}")
...
def get_shadow_params_for_pipeline(pipeline: str) -> Dict:
    ...
    logger.info(f"📋 Parámetros para pipeline '{pipeline}': {selected_params}")
...
def enhance_shadow_quality(img: Image.Image) -> Image.Image:
    ...
    try:
        ...
        logger.info("✨ Calidad de imagen mejorada")
        return img

    except Exception as e:
        logger.error(f"❌ Error mejorando calidad: {str(e)}")
        return img
...
def apply_professional_shadow(input_path: str, output_path: str,
    ...
    logger.info(f"🎨 Iniciando aplicación de sombra profesional")
    logger.info(f"📁 Input: {Path(input_path).name}")
    logger.info(f"🎯 Pipeline: {pipeline}, Tipo: {shadow_type}")

    try:
        # Detectar tipo automáticamente si es necesario
        if shadow_type == 'auto':
            shadow_type = detect_best_shadow_type(input_path)
            logger.info(f"🤖 Tipo detectado automáticamente: {shadow_type}")

        # Si el usuario eligió 'none', saltar todo el procesamiento de sombras
        if shadow_type == 'none':
            logger.info("❌ Usuario deshabilitó sombras (shadow_type='none'), copiando imagen original")
        ...
        if custom_params:
            ...
            logger.info(f"🔧 Parámetros personalizados aplicados: {custom_params}")
        ...
        if success:
            logger.info(f"✅ Sombra profesional aplicada exitosamente")
            logger.info(f"💾 Resultado guardado en: {Path(output_path).name}")
        else:
            logger.error(f"❌ Error aplicando sombra profesional")

        return success

    except Exception as e:
        logger.error(f"❌ Error en apply_professional_shadow: {str(e)}")
        return False