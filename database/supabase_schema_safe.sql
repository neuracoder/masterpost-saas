-- =====================================================
-- MASTERPOST.IO V2.0 - SAFE SUPABASE SCHEMA
...
-- Insert plan configurations (Update if exists)
...
on conflict (plan) do update set
    ...
    max_images_per_zip = excluded.max_images_per_zip,
    qwen_api_access = excluded.qwen_api_access,
...
-- Insert system configuration (Update if exists)
...
create index if not exists idx_user_usage_user_monthly on public.user_usage(user_id, year, month);
create index if not exists idx_user_usage_monthly_stats on public.user_usage(year, month);
...
create index if not exists idx_jobs_processing_method on public.jobs(processing_method);

create index if not exists idx_job_files_job_id on public.job_files(job_id);
create index if not exists idx_job_files_status on public.job_files(processing_status);

create index if not exists idx_api_keys_user_active on public.api_keys(user_id, is_active);
create index if not exists idx_api_keys_prefix on public.api_keys(key_prefix);

create index if not exists idx_billing_user_status on public.billing_transactions(user_id, status);
create index if not exists idx_billing_stripe_payment on public.billing_transactions(stripe_payment_intent_id);
create index if not exists idx_billing_period on public.billing_transactions(billing_period_start, billing_period_end);

create index if not exists idx_performance_metrics_name_timestamp on public.performance_metrics(metric_name, timestamp desc);
...
-- Apply updated_at trigger to relevant tables (Drop first to avoid duplication error)
...
drop trigger if exists update_job_files_updated_at on public.job_files;
...
-- This was the specific error reported by the user
